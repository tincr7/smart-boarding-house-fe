...
export default function BranchModal({ isOpen, onClose, onSubmit, initialData }: BranchModalProps) {
  ...
  // Effect: Reset form khi mở modal hoặc thay đổi chế độ (Thêm/Sửa)
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
...
const onFormSubmit = async (data: BranchFormValues) => {
    ...
    try {
      ...
      if (selectedFile) {
        const uploadRes: any = await uploadApi.upload(selectedFile, 'branches');
                imageUrl = uploadRes.url || uploadRes.secure_url || uploadRes.data?.url || uploadRes; 
      }

      // 2. Tạo Payload tường minh
      const payload = {
        name: data.name,
      ...
      console.error(error);
      alert('Lỗi khi tải lên hình ảnh hoặc xử lý dữ liệu.');
    } finally {
        ...
        <div className="px-8 py-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
          <div>
            <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter italic">
              {initialData ? 'Cập nhật Hồ sơ' : 'Khởi tạo Cơ sở mới'}
            </h2>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Hệ thống mở rộng chuỗi SmartHouse</p>
          </div>
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="p-8 space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
              <UploadCloud size={14} className="text-blue-500" /> Hình ảnh nhận diện
            </label>
            <div className="relative h-44 w-full rounded-[2rem] border-4 border-dashed border-slate-100 bg-slate-50 flex flex-col items-center justify-center overflow-hidden hover:border-blue-500 transition-all cursor-pointer group shadow-inner">
               {previewImage ? (
                 ...
                 <div className="text-center p-4">
                   <UploadCloud className="mx-auto h-10 w-10 text-slate-300 mb-2 group-hover:text-blue-500 transition-colors" />
          ...
          <div className="space-y-4">
            <div>
              ...
              <input 
                {...register('name')}
                className="w-full px-4 py-3 bg-slate-50 border-transparent rounded-xl text-xs font-bold uppercase outline-none focus:bg-white focus:ring-2 focus:ring-blue-500 transition-all shadow-sm focus:shadow-md" 
