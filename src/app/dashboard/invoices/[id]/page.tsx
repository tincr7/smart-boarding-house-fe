...
// 1. ĐÃ XÓA IMPORT SIDEBAR ĐỂ TRÁNH LẶP
...
import { 
  ...
  Trash2, Archive, Building2, MapPin, ReceiptText
} from 'lucide-react';
...
export default function InvoiceDetailPage({ params }: PageProps) {
  ...
  if (loading) return (
    <div className="h-[80vh] flex flex-col justify-center items-center bg-white gap-4">
  ...
  if (!invoice) return (
    <div className="h-[80vh] flex items-center justify-center text-slate-500 font-black uppercase tracking-widest text-xs bg-slate-50">
      Dữ liệu hóa đơn không tồn tại
    </div>
  );

  return (
    // 2. ĐÃ XÓA CẤU TRÚC KHUNG BAO NGOÀI (flex, Sidebar, ml-64...)
    // Layout cha sẽ tự động hiển thị Sidebar và margin nội dung
    <div className="p-8 space-y-8 bg-slate-50 min-h-screen print:bg-white print:p-0 selection:bg-blue-100">
      
      {/* Breadcrumbs */}
      <div className="inline-flex items-center px-4 py-2 bg-white rounded-xl border border-slate-100 shadow-sm print:hidden">
        <Breadcrumbs 
          items={[
